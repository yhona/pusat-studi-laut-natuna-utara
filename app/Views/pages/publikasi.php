...
<div class="bg-navy-950 text-white py-14 relative overflow-hidden border-b-2 border-gold-500">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <nav class="flex items-center space-x-2 text-xs text-gold-400 mb-2 font-medium">
            ...
            <span>/</span>
            <span class="text-slate-300">Publikasi Ilmiah</span>
        </nav>
        <h1 class="text-2xl sm:text-4xl font-extrabold tracking-tight text-white">Publikasi, Jurnal & Policy Brief</h1>
...
<div class="py-16 bg-slate-50" x-data="policyBriefModal()" @keydown.escape.window="if (isOpen) closeDownloadModal()">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div id="jurnal" class="space-y-6">
            <div class="border-b border-slate-200 pb-4">
                <span class="text-maritime-600 uppercase text-xs font-bold tracking-wider block">Publikasi Berkala Ilmiah</span>
                <h3 class="text-xl sm:text-2xl font-bold text-navy-950 mt-1">Jurnal Ilmiah Kelautan & Kemaritiman UMRAH</h3>
            </div>
                ...
                <div class="bg-white rounded-2xl p-7 border border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-4">
                    ...
                    <div class="pt-4 border-t border-slate-100 flex items-center justify-between">
                        <a href="<?= esc($j['link']) ?>" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 text-xs font-bold text-maritime-600 hover:text-navy-950 transition-colors">
                            Kunjungi Portal OJS <i class="fa-solid fa-arrow-up-right-from-square text-[10px]"></i>
                        </a>
                        <span class="text-[11px] text-slate-400">Terbit 2x Setahun</span>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
        ...
        <div id="policy-brief" class="scroll-mt-28 space-y-6">
            <div class="border-b border-slate-200 pb-4">
                <span class="text-gold-600 uppercase text-xs font-bold tracking-wider block">Rekomendasi Kebijakan</span>
                <h3 class="text-xl sm:text-2xl font-bold text-navy-950 mt-1">Policy Brief Kemaritiman Tropis</h3>
                <p class="text-slate-600 text-xs sm:text-sm mt-1">Ringkasan eksekutif berbasis bukti ilmiah untuk pertimbangan regulasi daerah dan nasional.</p>
            </div>
            ...
            <div class="space-y-4">
                <?php foreach ($policy_briefs as $pb): 
                    $meta = $metadataMap[$pb['number']] ?? [
                        ...
                        'file_size'       => '3.0 MB',
                        'pages'           => 16,
                        'recommendations' => ['Rekomendasi strategis tata kelola sumberdaya laut berkelanjutan.']
                    ];
                ...
                <div class="bg-white rounded-xl p-6 border border-slate-200 shadow-xs hover:border-maritime-500 hover:shadow-md transition-all flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="space-y-2 max-w-3xl">
                        ...
                        <span class="text-[11px] text-slate-500 block">Penyusun: <strong class="text-slate-700"><?= esc($pb['author']) ?></strong></span>
                    </div>
                    <div class="flex-shrink-0">
                        <button type="button" @click="openDownloadModal(<?= htmlspecialchars(json_encode($enrichedPb), ENT_QUOTES, 'UTF-8') ?>)"
                            ...
                            <i class="fa-regular fa-file-pdf text-rose-500 text-sm group-hover:scale-110 transition-transform"></i> Unduh PDF
                        </button>
                    </div>
                </div>
                <?php endforeach; ?>
    ...
    <div x-show="isOpen" x-cloak
         ...
         role="dialog"
         aria-modal="true"
         :aria-label="selectedPb ? selectedPb.title : 'Unduh Policy Brief'"
                ...
                <div class="flex items-start justify-between gap-4">
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs">
                            ...
                            <span class="text-slate-300" x-text="'Tahun ' + (selectedPb ? selectedPb.year : '')"></span>
                            <span class="text-slate-400">•</span>
                        ...
                        <p class="text-xs text-slate-300" x-text="'Penyusun: ' + (selectedPb ? selectedPb.author : '')"></p>
                    </div>
            ...
            <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto text-slate-700 text-xs sm:text-sm">
                ...
                <div class="bg-maritime-50/70 border border-maritime-200/70 rounded-xl p-4 space-y-3">
                    <div class="flex items-center gap-2 text-maritime-800 font-bold text-xs uppercase tracking-wider">
                        ...
                        <span>Ringkasan Eksekutif & Rekomendasi Kebijakan</span>
                    </div>
                    ...
                    <div class="pt-2 border-t border-maritime-200/60 space-y-1.5">
                        <span class="block text-[11px] font-bold text-navy-950 uppercase tracking-wide">3 Poin Rekomendasi Kunci:</span>
                ...
                <div x-show="downloadSuccess" x-cloak class="p-4 rounded-xl bg-emerald-50 border border-emerald-300 text-emerald-900 space-y-1">
                    <div class="flex items-center gap-2 font-bold text-xs">
                        ...
                        <span>Pengunduhan Dokumen Berhasil Diproses!</span>
                    </div>
                ...
                <form @submit.prevent="submitDownload()" class="space-y-4 pt-1">
                    <div class="border-b border-slate-100 pb-2">
                        <span class="font-bold text-navy-950 text-xs sm:text-sm block">Formulir Registrasi Pengunduh Dokumen</span>
                        <span class="text-[11px] text-slate-500">Data institusi digunakan untuk keperluan statistik dampak hilirisasi riset kelautan LPPM UMRAH.</span>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[11px] font-semibold text-slate-700 mb-1">Nama Lengkap & Gelar *</label>
                            <input type="text" x-model="form.name" required placeholder="Contoh: Dr. Ahmad Dahlan"
                        ...
                        <div>
                            <label class="block text-[11px] font-semibold text-slate-700 mb-1">Email Resmi / Institusi *</label>
                            <input type="email" x-model="form.email" required placeholder="nama@instansi.go.id"
                                   class="w-full px-3 py-2 text-xs rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-maritime-500 focus:border-transparent">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[11px] font-semibold text-slate-700 mb-1">Nama Instansi / Lembaga *</label>
                            <input type="text" x-model="form.institution" required placeholder="Contoh: Bappeda Kepri / Ditjen PRL KKP"
                                   class="w-full px-3 py-2 text-xs rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-maritime-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-[11px] font-semibold text-slate-700 mb-1">Kategori Lembaga</label>
                            <select x-model="form.category" class="w-full px-3 py-2 text-xs rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-maritime-500 focus:border-transparent bg-white">
                                <option value="pemerintah">Pemerintah Daerah / Kementerian (KKP/BRIN)</option>
                                <option value="universitas">Perguruan Tinggi / Lembaga Penelitian</option>
                                <option value="industri">BUMN / Sektor Swasta Maritim & Pelayaran</option>
                                <option value="lsm">Lembaga Swadaya Masyarakat / Komunitas Bahari</option>
                                <option value="umum">Masyarakat Umum & Praktisi</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3">
                        <a :href="'<?= base_url('unduhan/unduh/') ?>/' + selectedPb?.slug"
                           class="text-xs font-semibold text-slate-500 hover:text-maritime-600 transition-colors flex items-center gap-1.5">
                            <i class="fa-solid fa-arrow-down-long text-[10px]"></i> Unduh Langsung Tanpa Form
                        </a>
                        ...
                        <div class="flex items-center gap-2 w-full sm:w-auto justify-end">
                            ...
                            <button type="submit"
                                ...
                                <i class="fa-solid fa-download text-gold-400"></i> Unduh Dokumen PDF Lengkap
                            </button>
                        </div>
                    </div>
                </form>
