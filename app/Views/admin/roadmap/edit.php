...
<div class="space-y-6 max-w-4xl">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-extrabold text-navy-950">Edit Fase Roadmap Riset</h2>
            <p class="text-xs text-slate-500 mt-1">Perbarui periode, capaian target, atau status milestone kemaritiman.</p>
        </div>
    ...
    <!-- Form -->
    <form action="<?= base_url('admin/roadmap/update/' . $phase['id']) ?>" method="POST" class="space-y-6">
        <?= csrf_field() ?>
        ...
        <div class="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="status_en" placeholder="Contoh: In Progress"
                           value="<?= old('status_en', $phase['status_en']) ?>"
                           class="w-full text-xs rounded-xl border border-slate-300 p-2.5 focus:ring-2 focus:ring-maritime-500 focus:outline-none">
            ...
            <div class="space-y-4 pt-2">
                ...
                <div>
                    ...
                    <input type="text" name="title_en" placeholder="Contoh: Phase 1: Oceanographic Baseline Data Consolidation & Resource Mapping"
                           value="<?= old('title_en', $phase['title_en']) ?>"
                           class="w-full text-xs rounded-xl border border-slate-300 p-2.5 focus:ring-2 focus:ring-maritime-500 focus:outline-none">
                </div>
            </div>
            ...
            <div class="space-y-4 pt-2">
                <div>
                    ...
                    <textarea name="desc" rows="3" required placeholder="Uraikan fokus penelitian, output sains, dan implementasi pada fase ini..."
                              class="w-full text-xs rounded-xl border border-slate-300 p-2.5 focus:ring-2 focus:ring-maritime-500 focus:outline-none"><?= old('desc', $phase['desc']) ?></textarea>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">
                    ...
                    <textarea name="desc_en" rows="3" placeholder="Describe the research milestones, scientific outputs, and practical applications in this phase..."
                              class="w-full text-xs rounded-xl border border-slate-300 p-2.5 focus:ring-2 focus:ring-maritime-500 focus:outline-none"><?= old('desc_en', $phase['desc_en']) ?></textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center pt-2">
                <div>
                    ...
                    <input type="number" name="order_seq" value="<?= old('order_seq', $phase['order_seq']) ?>" min="0"
                           class="w-full text-xs rounded-xl border border-slate-300 p-2.5 focus:ring-2 focus:ring-maritime-500 focus:outline-none">
                </div>
                <div class="pt-5">
            ...
            <div class="pt-5 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" 
                    ...
                    <i class="fa-solid fa-save mr-1.5"></i> Perbarui Fase Roadmap
                </button>
            </div>
        </div>
    </form>
