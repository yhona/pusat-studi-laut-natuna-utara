...
<div class="space-y-6 max-w-3xl">
    ...
    <!-- Form -->
    <form action="<?= base_url('admin/statistik/update/' . $stat['id']) ?>" method="POST" class="space-y-6">
        <?= csrf_field() ?>
        ...
        <div class="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="icon" required placeholder="Contoh: fa-book-open-reader"
                           ...
                           class="w-full text-xs rounded-xl border border-slate-300 p-2.5 focus:ring-2 focus:ring-maritime-500 focus:outline-none">
                    <span class="text-[10px] text-slate-400">Pilihan populer: fa-book-open-reader, fa-certificate, fa-handshake-angle, fa-anchor-circle-check</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center pt-2">
                <div>
                    ...
                    <input type="number" name="order_seq" value="<?= old('order_seq', $stat['order_seq']) ?>" min="0"
                           class="w-full text-xs rounded-xl border border-slate-300 p-2.5 focus:ring-2 focus:ring-maritime-500 focus:outline-none">
                </div>
                <div class="pt-5">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" <?= old('is_active', $stat['is_active']) ? 'checked' : '' ?>
            ...
            <div class="pt-5 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" 
                    ...
                    <i class="fa-solid fa-save mr-1.5"></i> Perbarui Statistik
                </button>
            </div>
        </div>
    </form>
