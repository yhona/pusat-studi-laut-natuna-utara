...
<div class="space-y-6 max-w-3xl">
    ...
    <!-- Form -->
    <form action="<?= base_url('admin/mitra/update/' . $partner['id']) ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
        <?= csrf_field() ?>
        ...
        <div class="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs space-y-5">
            ...
            <div class="flex items-center gap-4 p-4 rounded-xl bg-slate-50 border border-slate-200/80">
                <div class="w-24 h-16 rounded-lg bg-white p-2 shrink-0 border border-slate-300 flex items-center justify-center">
                    <img src="<?= base_url(esc($partner['logo'])) ?>" alt="<?= esc($partner['name']) ?>" class="max-h-full max-w-full object-contain">
                </div>
                <div class="space-y-1 text-xs">
                    <span class="font-bold text-navy-950 block">Logo Saat Ini:</span>
                    <span class="font-mono text-slate-600 block"><?= esc($partner['logo']) ?></span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="short_name" value="<?= esc($partner['short_name']) ?>"
                           class="w-full text-xs rounded-xl border border-slate-300 p-2.5">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="category" required class="w-full text-xs rounded-xl border border-slate-300 p-2.5 bg-white">
                        <option value="pemerintah" <?= $partner['category'] === 'pemerintah' ? 'selected' : '' ?>>Pemerintah & Kementerian</option>
                        <option value="lembaga_riset" <?= $partner['category'] === 'lembaga_riset' ? 'selected' : '' ?>>Lembaga Riset Nasional</option>
                        <option value="universitas" <?= $partner['category'] === 'universitas' ? 'selected' : '' ?>>Universitas & Perguruan Tinggi</option>
                        <option value="internasional" <?= $partner['category'] === 'internasional' ? 'selected' : '' ?>>Organisasi / Lembaga Internasional</option>
                        <option value="industri" <?= $partner['category'] === 'industri' ? 'selected' : '' ?>>Industri & Badan Usaha Maritim</option>
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">
                    ...
                    <input type="url" name="website_url" value="<?= esc($partner['website_url']) ?>"
            ...
            <div class="p-4 rounded-xl bg-slate-50 border border-slate-200/80 space-y-3">
                <span class="text-xs font-bold text-navy-950 block">Ubah Logo Mitra</span>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="logo_preset" class="w-full text-xs rounded-xl border border-slate-300 p-2.5 bg-white">
                            <option value="">-- Tetap Gunakan Logo Saat Ini --</option>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center pt-2">
                <div>
                    ...
                    <input type="number" name="order_seq" value="<?= esc($partner['order_seq']) ?>" min="1" max="99" class="w-32 text-xs rounded-xl border border-slate-300 p-2.5">
                </div>
                <div class="flex items-center gap-2 pt-5">
                    <input type="checkbox" name="is_active" id="is_active" value="1" <?= $partner['is_active'] ? 'checked' : '' ?> class="rounded border-slate-300 text-maritime-600 focus:ring-maritime-500">
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2.5 rounded-xl bg-gold-500 hover:bg-gold-400 text-navy-950 text-xs font-bold shadow-md transition-all active:scale-[0.98]">
                    <i class="fa-solid fa-save mr-1.5"></i> Perbarui Mitra
                </button>
            </div>
        </div>
    </form>
